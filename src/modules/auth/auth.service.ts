...
const getOtp = (email: string, expireTime?: number) => {
...
const register = async (req: Request, res: Response) => {
  ...
  const responseObj = successResponse(
    "user is created succesfully, check your mailbox to activate your account",
  );
...
const verfiyemail = async (req: Request, res: Response, next: NextFunction) => {
...
const login = async (req: Request, res: Response, next: NextFunction) => {
  ...
  const accessToken = encoding(
    { email, id: user.getDataValue("id") },
    process.env.ACCESS_TOK!,
  );
  const refreshToken = encoding(
    { email, id: user.getDataValue("id") },
    process.env.REF_TOK!,
  );
  ...
  return res
    .status(200)
    .json(
      successResponse("logged in successfully", { accessToken, refreshToken }),
    );
};

const googleLogin = async (req: Request, res: Response, next: NextFunction) => {
  const user = req.user as UserType;
  if (!user) return next(new Error("user is undefined"));
  const accessToken = encoding(
...
const forgotPassword = async (
  ...
  return res
    .status(200)
    .json(
      successResponse("otp sent to your email, use it to set a new password"),
    );
};
...
const updateProfile = async (
  ...
  return res
    ...
    .json(
      successResponse(
        `profile updated successfully${email ? " check you inbox & use the otp sent to you to confirm updating your email" : "."}`,
      ),
    );
};
...
const updateEmail = async (req: Request, res: Response, next: NextFunction) => {
  ...
  return res
    ...
    .json(
      successResponse(
        `your email is updated successfully from ${u.email} to ${email}`,
      ),
    );
};
export {
